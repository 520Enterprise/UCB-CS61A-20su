...
class Place:
    ...
    def remove_insect(self, insect):
        ...
        Asks the insect to remove itself from the current place. This method exists so
            it can be enhanced in subclasses.
        """
        print("DEBUG:", insect, insect.place, self)
...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 9
            print("DEBUG:", place.ant, self)
            print("DEBUG:", place.ant.can_contain(self), self.can_contain(place.ant))
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        dis = 0
        while dis < self.min_range:
            if now_place is beehive:
                return None
            now_place = now_place.entrance
            dis += 1
        while dis <= self.max_range:
            if now_place is beehive:
                return None
            if now_place.bees:
                return rANTdom_else_none(now_place.bees)
            now_place = now_place.entrance
            dis += 1
...
class ContainerAnt(Ant):
    ...
    def contain_ant(self, ant):
        # BEGIN Problem 9
        "*** YOUR CODE HERE ***"
        assert self.can_contain(ant), "{} cannot contain {}".format(self, ant)
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, gamestate):
        ...
        if self is None:
            return
        if self.index != 1:
            print("DEBUG: ", self, self.index)
            self.reduce_armor(self.armor)
            return
        pos = self.place.exit
        while pos and pos is not AntHomeBase:
            ant = pos.ant
            if ant:
                print("DEBUG: ", ant, ant.doubled, ant.damage)
                if isinstance(ant, ContainerAnt):
                    if ant.contained_ant and not ant.contained_ant.doubled:
...
def make_scare(action, bee):
    ...
    def scare_action(gamestate):
        bee.direction = Direction.BACKWARD
        action(gamestate)
        bee.direction = Direction.FORWARD
...
def apply_status(status, bee, length):
    ...
    status_action = status(original_action, bee)
...
class LaserAnt(ThrowerAnt):
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem OPTIONAL
        return max(0, self.damage - distance * 0.2 - self.insects_shot * 0.05)
        # END Problem OPTIONAL
    ...
    def action(self, gamestate):
        ...
        for insect, distance in insects_and_distances.items():
            ...
            print("DEBUG: ", insect, distance, damage, self.insects_shot)
