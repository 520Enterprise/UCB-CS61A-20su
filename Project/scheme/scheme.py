...
def scheme_eval(expr, env, tail=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if isinstance(procedure, MacroProcedure):
            return scheme_eval(procedure.apply_macro(rest, env), env, tail)
...
def eval_all(expressions, env):
    ...
    while expressions is not nil:
        is_tail = expressions.rest is nil
        result = scheme_eval(expressions.first, env, is_tail)
...
def optimize_tail_calls(prior_eval_function):
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        if tail and not scheme_symbolp(expr) and not self_evaluating(expr):
            return Thunk(expr, env) # Pause recursion

        result = Thunk(expr, env)
        # BEGIN
        "*** YOUR CODE HERE ***"
        while isinstance(result, Thunk):
            result = prior_eval_function(result.expr, result.env, True)
        return result
        # END
