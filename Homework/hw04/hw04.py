...
def balanced(m):
    ...
    ls = end(left(m))
    rs = end(right(m))
    return balanced(ls) and balanced(rs) and \
        total_weight(ls) * length(left(m)) == total_weight(rs) * length(right((m)))
...
def totals_tree(m):
    ...
    ls = end(left(m))
    rs = end(right(m))
    return tree(total_weight(m), [totals_tree(ls), totals_tree(rs)])
...
def replace_leaf(t, find_value, replace_value):
    ...
    return tree(new_label, [replace_leaf(x, find_value, replace_value) for x in branches(t)])
...
def has_path(t, phrase):
    ...
    if len(phrase) == 1:
        return True
    return any([has_path(x, phrase[1:]) for x in branches(t)])
